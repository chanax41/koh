<?php 
    ...
    mysqli_query($conn, "SET NAMES 'utf8'");
        ...
        $sql = "UPDATE t_content
                    SET content_value = '$vdo_url'
                    WHERE content_key = 'vdo_url' and content_page = 'index'
                ";
        if(mysqli_query($conn, $sql)){
            $em = "บันทึกข้อมูลวิดีโอสำเร็จ";
            echo "<script>";
                echo "alert(\"$em\");"; 
                echo "window.location.href = 'update_index.php'";
            echo "</script>";
        }
         $sql = "UPDATE t_content
                    SET content_value = '$about'
                    WHERE content_key = 'about' and content_page = 'index'
                ";
        if(mysqli_query($conn, $sql)){
                    ...
                    if (in_array($img_ex_lc, $allowed_exs)) {
                        ...
                        if($result){
                            ...
                            $sql = "UPDATE t_content
                                        ...
                                        WHERE content_key = 'img_url' and content_page = 'index'
                                    ";
                            if(mysqli_query($conn, $sql)){
